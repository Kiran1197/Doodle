  ...
  .then((result)=>
  {
    if(result.length>0){
      ...
      .then(quantityRes=>{
        ...
        if(quantityRes.length > 0 && quantityRes[0].quantity>=req.body.quantity)
        {
...
exports.update_order=(req,res,next)=>{
    ...
    .exec()
    .then(result=>{
      if(result[0].cust_id==req.body.customerID)
      {
  ...
  var cust_id=req.params.cid;
    Order.aggregate([
      ...
      // {$lookup: {from: "users", 
          ...
          //  [{
          //      $match:{
          //          $expr:{$eq:["$_id","$username"]}
          //      }
          //  }], as: "details"}},
          {$lookup: {from: "users", localField: "cust_id", foreignField: "username", as: "details"}},
       {
              "$unwind": "$details"
          },{
      $group: {
...
exports.view_all_customer_orders=(req,res,next)=>{
  Order.aggregate([
    { $match: {"status":"active"} },
    {$lookup: {from: "users", localField: "cust_id", foreignField: "username", as: "details"}},
     {
